...
class FunkinLua {
	public static final Function_Stop:String = "####GALAXY_ENGINE_LUA.FUNCTION_STOP"; //Strings uses less memory than Int (compiled as Int64 on cpp)
	public static final Function_Continue:String = "####GALAXY_ENGINE_LUA.FUNCTION_CONTINUE"; //Strings uses less memory than Int (compiled as Int64 on cpp)
	public static final Function_StopLua:String = "####GALAXY_ENGINE_LUA.FUNCTION_STOP_LUA"; //Strings uses less memory than Int (compiled as Int64 on cpp)
	...
	public function new(script:String) {
		...
		set('ratingName', '');
		set('ratingFC', '');
		set('version', CoolUtil.getDisplayVersion(CoolUtil.parseVersion(MainMenuState.modVersion.trim())));
		...
		set('opponentMode', PlayState.opponentMode);
		...
		// Character shit
		set('boyfriendName', PlayState.SONG.boyfriend);
		set('dadName', PlayState.SONG.opponent);
		...
		Lua_helper.add_callback(lua, "getProperty", function(daVal:String) {
			var variable = daVal;
			if (variable == "songLength") variable = "needSongLength";
		...
		Lua_helper.add_callback(lua, "setProperty", function(daVal:String, value:Dynamic) {
			var variable = daVal;
			if (variable == "songLength") variable = "needSongLength";
			var killMe:Array<String> = variable.split('.');
			if(killMe.length > 1) {
		...
		Lua_helper.add_callback(lua, "precacheImage", function(name:String, format:String = '.png') {
			Paths.returnGraphic(name, format);
		});
		...
		Lua_helper.add_callback(lua, "setSongLength", function(newLength:Float, doLerp:Bool = true) {
			@:privateAccess
			PlayState.instance.setSongLength(newLength, doLerp);
		});
		...
		Lua_helper.add_callback(lua, "getLanguageString", function(stringToLookFor:String, ?language:String = "idkMan") {
			return Language.getString(stringToLookFor, language, true);
		});

		Lua_helper.add_callback(lua, "getOriginalWindowText", function() {
			lime.app.Application.current.window.title = GameConfig.defaultWindowName;
		});
		...
		Lua_helper.add_callback(lua, "preloadVideo", function(videoFile:String)
		{
			#if VIDEOS_ALLOWED
			if (FileSystem.exists(Paths.video(videoFile)))
			{
				PlayState.instance.preloadVideo(videoFile);
				return true;
			}
			else
			{
				luaTrace('preloadVideo: Video not found:  ' + videoFile, false, false, FlxColor.RED);
			}
			return false;
			#else
			if (PlayState.instance.endingSong)
		...
		Lua_helper.add_callback(lua, "startVideo", function(videoFile:String, x:Float, y:Float, camera:String)
		{
			#if VIDEOS_ALLOWED
			if (FileSystem.exists(Paths.video(videoFile)))
			{
				PlayState.instance.startVideo(videoFile, x, y, [cameraFromString(camera)]);
				return true;
			}
			else
			{
		...
		Lua_helper.add_callback(lua, "spawnCoinNote", function() {
			PlayState.instance.spawnCoinNote();
		});
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			case 'camload' | 'load': return PlayState.instance.camLoad;
			case 'camhud' | 'hud': return PlayState.instance.camHUD;
			case 'camvideo' | 'camvid' | 'video' | 'vid': return PlayState.instance.camVideo;
			case 'camextra' | 'extra': return PlayState.instance.camExtra;
			case 'camoverlay' | 'camover' | 'overlay' | 'over': return PlayState.instance.camOverlay;
...
class ModchartBackdrop extends flixel.addons.display.FlxBackdrop
{
	public var wasAdded:Bool = false;
